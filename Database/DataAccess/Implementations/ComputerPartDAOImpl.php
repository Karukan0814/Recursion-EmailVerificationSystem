...
class ComputerPartDAOImpl implements ComputerPartDAO
{
    ...
    {
        // 引数であるidが空でないか確認
        if ($id === null) throw new \Exception('Computer part specified has no ID.');

        //当該idが存在するか確認
        $current = $this->getById($id);
        if ($current === null) throw new \Exception(sprintf("Computer part %s does not exist.", $id));

        // 削除のSQLを発行
        $mysqli = DatabaseManager::getMysqliConnection();
        $result = $mysqli->prepareAndExecute("DELETE FROM computer_parts WHERE id = ?", 'i', [$id]);
        return $result;
    }
